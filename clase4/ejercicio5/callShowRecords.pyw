...
rowNo=1
sqlStatement="SELECT Correo, Contraseña FROM Usuarios"
...
class MyForm(QDialog):
    ...
    def ShowFirstRow(self):
        try:
            ...
            self.ui.labelResponse.setText("Hubo un error al acceder a la tabla")

    def ShowPreviousRow(self):
        global rowNo
        rowNo -= 1
        sqlStatement = "SELECT Correo, Contraseña FROM Usuarios where rowid = "+str(rowNo)
        cur.execute(sqlStatement)
        row = cur.fetchone()
        if row:
            ...
            rowNo += 1
            self.ui.labelResponse.setText("Esta es la primera fila ")
    ...
    def ShowNextRow(self):
        ...
        if row:
            ...
            rowNo -= 1
            self.ui.labelResponse.setText("Esta es la última fila ")
