...
class MyForm(QDialog):
    ...
    def createTable(self):
        ...
        try:
            ...
            self.ui.labelResponse.setText("Base de datos conectada")
            c = conn.cursor()
            tabledefinition += ");"
            c.execute(tabledefinition)
            self.ui.labelResponse.setText("La tabla ha sido creada correctamente")
        except Error as e:
            self.ui.labelResponse.setText("Hubo un error creando la tabla")
        finally:
            conn.close()
